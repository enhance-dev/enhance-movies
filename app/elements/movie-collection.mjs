...
export default function MovieCollection ({ html, state }) {
  const { store } = state
  const shows = store.shows.results.map(({ id, poster_path, title, vote_average }) => `<movie-poster id='${id}' poster_path='${poster_path}' title='${formatTitle(title)}' vote_average='${vote_average}'></movie-poster>`).join('')
